  ...
  return (
    <div className="group relative bg-slate-800/50 hover:bg-slate-800 border border-slate-700 hover:border-indigo-500/50 rounded-xl p-5 transition-all duration-300 shadow-md hover:shadow-xl">
      ...
      <div className="flex items-start gap-4">
        {/* Provider/Schema Logo */}
        {attestation.schemaLogo ? (
            <img 
                src={attestation.schemaLogo} 
                alt={attestation.provider} 
                className="w-12 h-12 rounded-full object-cover border border-slate-700 bg-slate-900"
            />
        ) : (
            <div className="p-3 rounded-full bg-slate-700/50 text-indigo-400 flex items-center justify-center w-12 h-12">
      ...
      <div className="mt-4 pt-4 border-t border-slate-700/50 flex justify-between items-center">
        ...
        <div className="flex items-center gap-2">
            ...
            {/* On-Chain Badge */}
            <span className="text-[10px] font-mono text-emerald-500 bg-emerald-900/20 px-1.5 py-0.5 rounded border border-emerald-500/20" title="Data sourced from EVM Blockchain">
                🟢 On-Chain
            </span>
        </div>
