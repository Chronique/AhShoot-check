...
// Handle Environment Variable safely for Vite (using the define shim in vite.config.ts) or Next.js
const getEnvUrl = () => {
  if (typeof process !== 'undefined' && process.env && process.env.NEXT_PUBLIC_URL) {
    return process.env.NEXT_PUBLIC_URL;
  }
  // Fallback if env is missing
  return "https://ah-shoot-check.vercel.app/";
};
